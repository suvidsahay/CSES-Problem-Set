...
int query(int tree[], int l, int r, int start = 0, int end = n - 1, int idx = 1) {
    ...
    if(l <= start && r >= end) {
        ...
        int mid = (start + end) / 2;
        return min(query(tree, l, r, start, mid, idx * 2), query(tree, l, r, mid + 1, end, idx * 2 + 1));
    }
}

void update(int tree[], int pos, int val, int start = 0, int end = n - 1, int idx = 1) {
    ...
    if(start == end) {  
        ...
        int mid = (start + end) / 2;
        update(tree, pos, val, start, mid, idx * 2);
        update(tree, pos, val, mid + 1, end, idx * 2 + 1);
